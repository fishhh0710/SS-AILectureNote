...
async def detect_opencv(file: UploadFile = File(...)):
    if not opencv or not config:
        raise HTTPException(status_code=503, detail="OpenCV detection module is not available")
    ...
    try:
        ...
        if not out_json_path.exists():
            raise HTTPException(status_code=500, detail="Resulting JSON coordinates file was not created by OpenCV module")
...
async def detect_opencv_morph(file: UploadFile = File(...)):
    if not opencv_morph or not config:
        raise HTTPException(status_code=503, detail="OpenCV morphology module is not available")
    ...
    try:
        ...
        if not out_json_path.exists():
            raise HTTPException(status_code=500, detail="Resulting JSON coordinates file was not created by morphology module")
...
async def detect_sam(file: UploadFile = File(...)):
    ...
    if not checkpoint_path.exists():
        ...
        try:
            Mobile_SAM.download_checkpoint(str(checkpoint_path), config.SAM_CHECKPOINT_URL)
        except Exception as e:
            logger.error(f"Failed to download checkpoint: {e}")
            raise HTTPException(status_code=500, detail=f"Failed to download MobileSAM model checkpoint: {e}")
    ...
    try:
        ...
        if not out_json_path.exists():
            raise HTTPException(status_code=500, detail="Resulting coordinates JSON not generated by MobileSAM")
...
async def detect_gemini(file: UploadFile = File(...)):
    if not gpt_bbox or not config:
        raise HTTPException(status_code=503, detail="GPT matching module is not available")
    ...
    try:
        ...
        if not out_json_path.exists():
            raise HTTPException(status_code=500, detail="GPT matching output coordinates not generated")
...
async def detect_gpt(file: UploadFile = File(...)):
    ...
    try:
        ...
        if not out_json_path.exists():
            raise HTTPException(status_code=500, detail="GPT output coordinates not generated")
...
async def draw_bounding_boxes(
    ...
    if not draw_bbox_1 or not config:
        raise HTTPException(status_code=503, detail="Draw bounding box module is not available")
    ...
    try:
        ...
        if not out_image_path.exists():
            raise HTTPException(status_code=500, detail="Final annotated image not generated")
...
async def detect_pipeline(file: UploadFile = File(...)):
    if not all([query_gpt, opencv, opencv_morph, gpt_bbox, config]):
        raise HTTPException(status_code=503, detail="Required modules (gpt, opencv, morphology, or gpt_bbox) are not loaded")
    ...
    async with pipeline_lock:
        ...
        try:
            ...
            if not out_gpt_json.exists():
                raise HTTPException(status_code=500, detail="GPT recommendations file was not generated")
            ...
            if not out_opencv_json.exists():
                raise HTTPException(status_code=500, detail="OpenCV region coordinates were not generated")
            ...
            if not out_morph_json.exists():
                raise HTTPException(status_code=500, detail="OpenCV morphology coordinates were not generated")
                
            # 5. Run Step 3: GPT Matching
            gpt_bbox.main()
            if not out_gemini_json.exists():
                raise HTTPException(status_code=500, detail="GPT matching coordinate mapping results were not generated")
            ...
            coordinates_data = {
                "OPENCV": opencv_coords,
                "OPENCV_MORPH": morph_coords
            }
            ...
            for item in decisions:
                ...
                results.append({
                    "label": label,
                    "color": color_name,
                    "box": [float(x_min_merged), float(y_min_merged), float(x_max_merged), float(y_max_merged)]
                })
            ...
            paths_to_clean = [
                input_image_path, out_gpt_json, out_opencv_json, out_morph_json,
                out_gemini_json, opencv_output_image, morph_output_image
            ]
...
async def detect_agent_pipeline(
    file: UploadFile = File(...),
    targets: str = Form(...) # JSON string representing the targets array from the agent
):
    if not all([opencv, opencv_morph, gpt_bbox, config]):
        raise HTTPException(status_code=503, detail="Required modules (opencv, morphology, or gpt_bbox) are not loaded")
    ...
    async with pipeline_lock:
        ...
        try:
            ...
            try:
                ...
                raise HTTPException(status_code=400, detail=f"Invalid targets JSON format: {e}")
