...
class PageAnnotationManager {
  ...
  Future<void> setGenerationStatus(String status) => dbHelper.setAnnotationStatus(pdfId, status);
  Future<void> deleteGenerationStatus() => dbHelper.deleteAnnotationStatus(pdfId);
  ...
  Future<void> _discardIfInterrupted() async {
    try {
      ...
      if (status == 'generating') {
        ...
        await dbHelper.deleteAnnotationStatus(pdfId);
        // ignore: avoid_print
        print('DEBUG [Agent]: Discarded partial bounding boxes on close for PDF $pdfId');
      }
    } catch (e) {
      // ignore: avoid_print
