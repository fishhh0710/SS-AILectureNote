  ...
  const TranscriptPage(
    ...
    sections: [
      TranscriptSection(
        ...
        content: "我想上次我們應該是談到投影片第18頁了，基本上我們把一些不同的指令，R-format 的指令需要的哪些 Datapath 的 Component，還有 Load/Store 需要的 Datapath Component，還有 Branch 的指令需要的 Datapath Component 跟各位介紹可以嗎？",
      ),
    ],
  ),
  const TranscriptPage(
  ...
  const TranscriptPage(
    ...
    sections: [
      TranscriptSection(
        ...
        content: "我們在第五章再跟各位介紹 Cache 跟主記憶體之間的一些關聯性。另外就是我們需要透過用 Multiplexer，當你的資料來源有多個資料來源的話，那你怎麼從那些資料來源裡頭去選？所以你的電路裡面還是需要用到一些 Multiplexer。好，那我們現在把這些 Component 把它 Connect 在一起，那這裡的話主要是針對 R-type 的指令跟 Load/Store 的指令看到的 Datapath。當然這邊的 Datapath 沒有把那個 Instruction memory 放進來，那主要是 Focus 在這個 Register file 跟這個你的這個 Data memory。",
      ),
    ],
  ),
  const TranscriptPage(
  ...
  const TranscriptPage(
    ...
    sections: [
      TranscriptSection(
        ...
        content: "好，那我們總共有談 R-format 的指令有四個：有 add、有 sub，還有 and 跟 or。那另外有 ld 的指令、有 sd 的指令，那另外還有一個 beq 的指令那分別屬於這四種...這四種 type。這三十二個 bit 分別會拆成什麼樣的欄位，你可以再回來參考 23 頁這邊。",
      ),
    ],
  ),
  const TranscriptPage(
  ...
  const TranscriptPage(
    ...
    sections: [
      TranscriptSection(
        ...
        content: "好，所以我們的電路應該現在就長成像 29 頁這樣子。有一個 Control 的電路，然後這個 control 另外還有一個 ALU control。大概是這兩個電路來產生這些控制訊號（這個 control 就是我講的 main control）。",
      ),
    ],
  ),
  const TranscriptPage(
  ...
  const TranscriptPage(
    ...
    sections: [
      TranscriptSection(
        ...
        content: "好，下面我們來看一下這個 Single-cycle processor 它的一些不同型態指令的執行過程。看看這 Add (R-type) 指令的過程：從 PC 把指令 Fetch 出來，PC Increment +4，然後取出 X2、X3 到 ALU 去運算，最後寫回 Destination Register X1。在這投影片 33 頁我們把相關路徑用較深的顏色強調顯示，也就是這些加深的粗線表示當下這個時鐘週期真正的 Data flow！",
      ),
    ],
  ),
  const TranscriptPage(
    ...
    sections: [
      TranscriptSection(
        ...
        content: "那如果今天是執行 ld 的指令呢？這 35 頁的系統圖面上描繪 Load 指令，資料經過 Sign-extend 再由 ALU 計算位置，並從 Data memory 把這筆資料取回交給暫存器 X1。你可以明顯對比出 MUX 的不同路徑！",
      ),
    ],
  ),
  const TranscriptPage(
    ...
    sections: [
      TranscriptSection(
        ...
        content: "如果碰到 beq 這張投影片 37 頁，Data 根本沒進去 register write 或者 data memory，而是跑到上面去算 Target address 和判斷 Zero 來選擇 PC 源頭。這就描繪了 Branch 指令的資料跑的軌跡。",
      ),
    ],
  ),
  const TranscriptPage(
